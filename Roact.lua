    ...
    function Config.new()
        ...
        self._currentConfig = setmetatable({}, {
            __index = function(_, key)
                local message = (
[[Invalid global configuration key %q. Valid configuration keys are: %s]]):format(tostring(key), table.concat(defaultConfigKeys, ', '))
    ...
    function Config:set(configValues)
        for key, value in pairs(configValues)do
            if defaultConfig[key] == nil then
                local message = (
[[Invalid global configuration key %q (type %s). Valid configuration keys are: %s]]):format(tostring(key), typeof(key), table.concat(defaultConfigKeys, ', '))

                error(message, 3)
            end
            if typeof(value) ~= 'boolean' then
                local message = (
[[Invalid value %q (type %s) for global configuration key %q. Valid values are: true, false]]):format(tostring(value), typeof(value), tostring(key))
...
do
    ...
    local function createReconciler(renderer)
        ...
        local function updateVirtualNodeWithChildren(
            virtualNode,
            hostParent,
            newChildElements
        )
        ...
        local function updateVirtualNodeWithRenderResult(
            virtualNode,
            hostParent,
            renderResult
        )
        ...
        local function unmountVirtualTree(tree)
            ...
            if config.typeChecks then
                ...
                assert(internalData.mounted, 
[[Cannot unmounted a Roact tree that has already been unmounted]])
            end
...
do
    ...
    function BindingInternalApi.map(upstreamBinding, predicate)
        ...
        function impl.subscribe(callback)
            return BindingInternalApi.subscribe(upstreamBinding, function(
                newValue
            )
        ...
        function impl.update(_newValue)
            error(
[[Bindings created by Binding:map(fn) cannot be updated directly]], 2)
        end
        function impl.getValue()
        ...
        if config.typeChecks then
            ...
            for key, value in pairs(upstreamBindings)do
                if Type.of(value) ~= Type.Binding then
                    local message = (
[[Expected arg #1 to contain only bindings, but key %q had a non-binding value]]):format(tostring(key))
        ...
        function impl.update(_newValue)
            error(
[[Bindings created by joinBindings(...) cannot be updated directly]], 2)
        end
        function impl.getValue()
            return getValue()
        end
...
[[Name can not be specified as a prop to a host component in Roact.]])
...
do
    ...
    local Type = __DARKLUA_BUNDLE_MODULES.f
    local config = __DARKLUA_BUNDLE_MODULES.b.get()
    local multipleChildrenMessage = 'The prop `Roact.Children` was defined but was overridden by the third parameter to createElement!\nThis can happen when a component passes props through to a child element but also uses the `children` argument:\n\n\tRoact.createElement("Frame", passedProps, {\n\t\tchild = ...\n\t})\n\nInstead, consider using a utility function to merge tables of children together:\n\n\tlocal children = mergeTables(passedProps[Roact.Children], {\n\t\tchild = ...\n\t})\n\n\tlocal fullProps = mergeTables(passedProps, {\n\t\t[Roact.Children] = children\n\t})\n\n\tRoact.createElement("Frame", fullProps)'
...
do
    ...
    local function createConsumer(context)
        ...
        function Consumer:didMount()
            if self.contextEntry ~= nil then
                self.disconnect = self.contextEntry.onUpdate:subscribe(function(
                    newValue
                )
